...
10. Close Steam (best effort), backup configs, upsert Steam shortcuts, update collections (localconfig + cloud namespace), copy cached artwork into Steam grid, reopen Steam
   ...
   - Linux Steam Deck default shortcut policy:
    ...
    - Linux Steam Deck template sync for managed `Wii` and `N3DS` shortcuts (`GC` is intentionally excluded):
      ...
      - by default, those managed Deck input app entries also set `DisableCloud = 1` to reduce first-launch Steam cloud template resets (disable with `GAMEHUB_DECK_DISABLE_STEAM_CLOUD_INPUT=false`)
