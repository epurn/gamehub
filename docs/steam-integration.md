...
- On macOS, when a managed shortcut target resolves to an app bundle executable (`*.app/Contents/MacOS/...`), sync writes one shared launcher script beside `state.json` and passes only the managed `title_id` through Steam. The launcher forces native Apple Silicon Python execution before `shortcut-launch` runs `open -W -a <App> --args ...`, so bundle-safe launch and post-exit waiting both stay intact without per-title launcher churn.
