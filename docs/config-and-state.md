...
- Managed Azahar profiles and assisted `qt-config.ini` convergence set `Shortcuts\Main%20Window\Exit%20Azahar\KeySeq=Esc` with `Shortcuts\Main%20Window\Exit%20Azahar\KeySeq\default=false`, keep the legacy `Exit Citra` alias aligned for older configs, and clear `Shortcuts\Main%20Window\Exit%20Fullscreen\KeySeq` with `Shortcuts\Main%20Window\Exit%20Fullscreen\KeySeq\default=false` so `Esc` exits the managed session instead of only leaving fullscreen.
- Managed Azahar controller profiles and assisted `qt-config.ini` convergence also normalize `profiles\1\circle_pad` and `profiles\1\c_stick` to deterministic SDL bindings with `deadzone:0.220000` (aligned with the Steam Deck template's tested inner-deadzone behavior), so managed sessions filter common stick ghost-input without per-title tuning.
