...
- `GAMEHUB_AZAHAR_MOUSE_BRIDGE`: enables/disables Azahar launch-time right-stick mouse + `R2` left-click bridging (`true` by default). This is global, but Steam Deck Linux hosts still hard-disable the bridge even when the env var is left enabled.
- `GAMEHUB_AZAHAR_MOUSE_BRIDGE_EVENT_DEVICE`: optional explicit Linux `/dev/input/event*` device for the Azahar mouse bridge when auto-resolution does not pick the intended controller.
...
- `GAMEHUB_AZAHAR_MOUSE_BRIDGE=true` enables Azahar launch-time mouse bridging on Windows, macOS, and supported non-Steam-Deck Linux hosts; if the platform backend or controller access is unavailable, launch continues without synthetic mouse input.
...
- On non-Steam-Deck Linux hosts such as Bazzite, Azahar launch-time mouse bridging is capability-gated and fail-open:
  - right stick drives a virtual OS mouse and `R2` holds left click for the launch session only
  - requires a readable matching `/dev/input/event*` controller device and writable `/dev/uinput`
  - `GAMEHUB_AZAHAR_MOUSE_BRIDGE_EVENT_DEVICE` can pin the event device explicitly when auto-resolution is not sufficient
- On Steam Deck hosts, GAMEHUB intentionally leaves the existing built-in-controller path unchanged and does not start the Linux Azahar mouse bridge.
