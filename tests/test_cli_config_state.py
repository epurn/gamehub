...
from gamehub_cli.config_flow import run_config_init, run_config_verify
...
def test_config_init_defaults_to_existing_resolution_target(monkeypatch, workspace_tempdir) -> None:
    with workspace_tempdir("gamehub-cli-config-init-home-") as temp_root:
        home = temp_root / "home"
        home_config = home / ".gamehub" / "config.toml"
        home_config.parent.mkdir(parents=True, exist_ok=True)
        home_config.write_text(
            "\n".join(
                [
                    "[server]",
                    'url = "http://existing.invalid:9000"',
                    "",
                    "[paths]",
                    'gamehub_dir = "/existing/gamehub"',
                ]
            )
            + "\n",
            encoding="utf-8",
        ...
        exit_code = run_config_init(
            ...
            steam_id=None,
            controller_launch_autoconfig=None,
            save_sync_enabled=None,
            save_sync_mode=None,
            save_sync_conflict_policy=None,
            interactive=False,
        )

        rendered = home_config.read_text(encoding="utf-8")
        assert exit_code == 0
        assert 'url = "http://existing.invalid:9000"' in rendered
        assert 'gamehub_dir = "/existing/gamehub"' in rendered
        assert rendered.startswith("# Generated by `gamehub config init`.")


def test_config_init_overwrite_creates_backup_and_atomic_replace(monkeypatch, workspace_tempdir) -> None:
    with workspace_tempdir("gamehub-cli-config-init-overwrite-") as temp_root:
        config_path = temp_root / "config.toml"
        config_path.write_text(
            "\n".join(
                [
                    "[server]",
                    'url = "http://old.invalid:8000"',
                    "",
                    "[paths]",
                    'gamehub_dir = "/old/gamehub"',
                ]
            )
            + "\n",
            encoding="utf-8",
        ...
        assert 'url = "http://old.invalid:8000"' in backups[0].read_text(encoding="utf-8")
        rendered = config_path.read_text(encoding="utf-8")
        assert 'url = "http://new.invalid:9000"' in rendered
        assert 'gamehub_dir = "/new/gamehub"' in rendered
        assert "launch_autoconfig = false" in rendered
...
def test_config_verify_reports_actionable_errors(workspace_tempdir) -> None:
    with workspace_tempdir("gamehub-cli-config-verify-") as temp_root:
        ...
        with pytest.raises(ValueError, match="Config file not found"):
            run_config_verify(config_path=missing_config)

        invalid_config = temp_root / "config.toml"
        invalid_config.write_text(
            "\n".join(
                [
                    "[steam]",
                    'steam_id = "not-numeric"',
                ]
            )
            + "\n",
            encoding="utf-8",
        ...
        with pytest.raises(ValueError, match="Configured steam_id is not numeric"):
            run_config_verify(config_path=invalid_config)
