...
def test_build_shortcut_specs_linux_normalizes_retroarch_core_token(monkeypatch, workspace_tempdir) -> None:
    with workspace_tempdir("gamehub-sync-shortcuts-linux-") as temp_root:
        ...
        assert "/var/home/deck/.var/app/org.libretro.RetroArch/config/retroarch/cores/fceumm_libretro.so" in specs[0].launch_options
...
def test_build_shortcut_specs_wrapper_prefers_gamehub_console_script_on_linux(monkeypatch, workspace_tempdir) -> None:
    with workspace_tempdir("gamehub-sync-shortcuts-linux-gamehub-wrapper-") as temp_root:
        config = GamehubConfig(
            server_url="http://localhost:8000",
            library_dir=temp_root / "library",
            firmware_dir=temp_root / "firmware",
        ...
        monkeypatch.setattr("gamehub_cli.sync.steam_stage.sys.platform", "linux")
        monkeypatch.setattr("gamehub_cli.sync.steam_stage.sys.executable", "/home/deck/gamehub/venv/bin/python3")
        monkeypatch.setattr(
            "gamehub_cli.sync.steam_stage.resolve_rom_destination",
            lambda **kwargs: Path("/var/home/deck/GameHub/roms/PS2/Gran Turismo 4.iso"),
        )
        ...
        def _patched_exists(path_obj: Path) -> bool:
            if str(path_obj).replace("\\", "/") == "/home/deck/gamehub/venv/bin/gamehub":
                return True
        ...
        assert len(specs) == 1
        assert _normalize_path_token(specs[0].exe) == "/home/deck/gamehub/venv/bin/gamehub"
...
def test_build_shortcut_specs_wrapper_prefers_gamehub_from_path(monkeypatch, workspace_tempdir) -> None:
    with workspace_tempdir("gamehub-sync-shortcuts-linux-gamehub-path-wrapper-") as temp_root:
        config = GamehubConfig(
            server_url="http://localhost:8000",
            library_dir=temp_root / "library",
            firmware_dir=temp_root / "firmware",
        ...
        monkeypatch.setattr("gamehub_cli.sync.steam_stage.shutil.which", lambda name: "/usr/local/bin/gamehub")
        monkeypatch.setattr(
            "gamehub_cli.sync.steam_stage.resolve_rom_destination",
            lambda **kwargs: Path("/var/home/deck/GameHub/roms/PS2/Gran Turismo 4.iso"),
        )
        ...
        assert len(specs) == 1
        assert _normalize_path_token(specs[0].exe) == "/usr/local/bin/gamehub"
        assert specs[0].launch_options.startswith("shortcut-launch --payload ")
