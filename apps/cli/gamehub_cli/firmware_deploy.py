...
def _configure_retroarch_runtime(
    ...
    if dry_run:
        if verbose:
            ...
            writer(
                "retroarch\tdry-run\tcore-options\t"
                f"{core_options_path}\tpsx_controller=AnalogController"
            )
    ...
    existing_remap = {key: _read_simple_cfg_key(lines, key) for key in _RETROARCH_REMAP_PORT_KEYS} if not is_windows else {}
    ...
    if verbose:
        ...
        writer(
            "retroarch\tconfigured\tcore-options\t"
            f"{core_options_path}\tpsx_controller=AnalogController"
        )
...
def _configure_dolphin_runtime(
    ...
    for ini_path in ini_paths:
        ...
        if verbose:
            writer(
                "dolphin\tconfigured\t"
                f"{ini_path}\tfullscreen=true\tconfirm_stop=false\tbackground_input=true"
            )
    return runtime_user_dir / "Config" / "Dolphin.ini"
