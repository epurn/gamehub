...
def resolve_retroarch_paths(
    ...
    for cfg_path in retroarch_cfg_candidates(
        ...
        if raw_core:
            candidate = _normalize_retroarch_cfg_path(raw_core, cfg_path=cfg_path) or _path_with_tilde_expanded(
                raw_core
            )
        ...
        if raw_info:
            candidate = _normalize_retroarch_cfg_path(raw_info, cfg_path=cfg_path) or _path_with_tilde_expanded(
                raw_info
            )
            if not candidate.is_absolute():
                candidate = cfg_path.parent / candidate
