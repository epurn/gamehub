...
def resolve_retroarch_system_dirs(config: GamehubConfig | None = None) -> list[Path]:
    ...
    for cfg_path in retroarch_cfg_candidates_for_config(config=config):
        ...
        if raw.lower() == "default":
            ...
            continue
        if _OS_NAME == "nt" and raw.startswith((":\\", ":/")):
    ...
    if retroarch_exe.exists() and (_OS_NAME == "nt" or retroarch_exe.name.casefold() == "retroarch.exe"):
        values.append(retroarch_exe.parent / "system")
    elif _SYS_PLATFORM.startswith("linux") and prefer_flatpak:
    ...
    if _SYS_PLATFORM.startswith("linux") and prefer_flatpak:
...
def pcsx2_ini_candidates(config: GamehubConfig | None = None) -> list[Path]:
    ...
    user_profile = os.environ.get("USERPROFILE")
    if _OS_NAME == "nt" and user_profile:
